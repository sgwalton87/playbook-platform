...
export function createContextRefreshApproval(input: {
  ...
  const body: Omit<ContextRefreshApprovalRecord, "digest"> = {
    approval_id: `CONTEXT-REFRESH-APPROVAL-${artifactDigest({
      repository: input.reconciliation.current_snapshot.repositoryIdentity,
      branch: input.reconciliation.current_snapshot.git.branch,
      commit: input.reconciliation.current_snapshot.git.commitSha,
...
export function validateAppliedContextRefreshApproval(
  ...
  if (
    ...
    approval.decision !== "APPROVED" ||
    !approval.applied_at ||
    !approval.resulting_context_identity
  ) {
    findings.push("Refresh approval has not completed its governed application.");
  }
  if (currentContextIdentity !== approval.resulting_context_identity) {

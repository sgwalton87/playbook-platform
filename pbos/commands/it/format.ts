...
export function formatItCommand(result: FounderOperatingLoopResult): string {
  ...
  const baselineBoundary =
    result.mission_control.boundary_type === "BASELINE_ACTIVATION";
  const trustedContextDisplay =
    result.mission_control.context_status === "TRUSTED"
      ? "ACTIVE"
      : baselineBoundary && humanEvidenceComplete
        ? "READY"
  ...
  return [
    ...
    `Boundary Type: ${result.mission_control.boundary_type}`,
    ...(baselineBoundary
      ? [`Repository Baseline: ${
          result.mission_control.change_boundary_status === "APPROVED"
            ? "VALID"
      ...
      : []),
    `Human Evidence: ${humanEvidenceComplete ? "VALID" : "MISSING"}`,

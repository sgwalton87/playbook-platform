...
for (const viewport of [{ name: "desktop", width: 1440, height: 900 }, { name: "mobile", width: 390, height: 844 }]) {
  ...
  test(`Academic Readiness renders the canonical evidence-to-action journey on ${viewport.name}`, async ({ page }) => {
    await page.setViewportSize(viewport);
    await page.goto("/academic-readiness");
    await expect(page.locator('[data-testid="academic-readiness"][data-visual-canon="PGAR-001"]')).toBeVisible();
    await expect(page.getByRole("heading", { name: /Your transcript should tell you/i })).toBeVisible();
    await expect(page.getByRole("link", { name: "Open transcript" })).toHaveAttribute("href", "/transcript");
    await expect(page.getByRole("link", { name: "Open support network" })).toHaveAttribute("href", "/support-network");
    await expect(page.getByRole("link", { name: "Open Oracle" })).toHaveCount(0);
    expect(await page.evaluate(() => document.documentElement.scrollWidth <= document.documentElement.clientWidth)).toBe(true);
    const accessibility = await new AxeBuilder({ page }).analyze();
    expect(accessibility.violations.filter((item) => ["serious", "critical"].includes(item.impact ?? ""))).toEqual([]);
    await page.screenshot({ path: `artifacts/pbos-acceptance/ui-recovery-academic-readiness-${viewport.name}.png`, fullPage: true });
  });
}

...
async function advanceToWarning(page: Page): Promise<void> {
...
test("Session Timeout protects an idle authenticated session on desktop and mobile", async ({
  context,
  page,
}) => {
  ...
  if (!user) {
    ...
    if (created.error || !created.data.user) {
      throw created.error ?? new Error("Synthetic Session Timeout user creation failed.");
    }
  ...
  await advanceToWarning(page);
  await page.screenshot({
    path: `${artifacts}/048-phase-01-item-session-timeout-desktop.png`,
    fullPage: true,
  });
  ...
  await writeFile(
    `${artifacts}/048-phase-01-item-session-timeout-accessibility.json`,
    JSON.stringify(accessibility, null, 2)
  );
  expect(blocking).toEqual([]);
  await page.screenshot({
    path: `${artifacts}/048-phase-01-item-session-timeout-mobile.png`,
    fullPage: true,
  });
  ...
  await context.tracing.stop({
    path: `${artifacts}/048-phase-01-item-session-timeout-trace.zip`,
  });
  await writeFile(
    `${artifacts}/048-phase-01-item-session-timeout.json`,
    JSON.stringify({
      schemaVersion: 1,
      journeyId: "048-PHASE-01-ITEM-SESSION-TIMEOUT",
      commit,
      checks: [

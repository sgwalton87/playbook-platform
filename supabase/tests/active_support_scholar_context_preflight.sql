...
select pg_get_functiondef('public.set_active_support_scholar_context(uuid)'::regprocedure) ~ 'supporter_id\s*=\s*actor_id'
   and pg_get_functiondef('public.set_active_support_scholar_context(uuid)'::regprocedure) ~ 'status\s*=\s*''active''' as active_relationship_required \gset
\if :active_relationship_required \else \echo 'setter must require active supporter relationship' \quit 1 \endif

select pg_get_functiondef('public.get_active_support_scholar_context()'::regprocedure) ~ 'r\.status\s*=\s*''active''' as getter_rechecks_active \gset

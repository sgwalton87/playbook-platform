-- Governed human review authority for role-verification evidence.
--
-- This is intentionally separate from moderation authority. Platform Founder/Admin
-- reviewers may inspect and transition only the allowlisted verification/readiness
-- requests below. The function cannot grant arbitrary profile or Scholar-data
-- access; downstream role/relationship/scope gates remain independently required.

create or replace function private.current_user_is_verification_reviewer()
returns boolean
language sql
stable
security definer
...
revoke all on function private.current_user_is_verification_reviewer() from public, anon, authenticated;
...
create table if not exists public.verification_review_events (
  ...
  new_status text not null check (new_status in ('under_review','approved','rejected')),
  review_notes text,
  created_at timestamptz not null default now()
);

alter table public.verification_review_events enable row level security;
revoke all on public.verification_review_events from anon, authenticated;
grant select on public.verification_review_events to authenticated;

drop policy if exists "Verification reviewers can view review events" on public.verification_review_events;
create policy "Verification reviewers can view review events"
on public.verification_review_events
for select
to authenticated
using ((select private.current_user_is_verification_reviewer()));
...
begin
  ...
  case requested_type
    when 'coach' then
      ...
      update public.coach_verification_requests set status=requested_decision, reviewed_at=final_at, review_notes=requested_notes, updated_at=now() where id=requested_id;
    when 'educator' then
      ...
      if old_status not in ('pending','under_review') then raise exception 'request is not reviewable'; end if;
      update public.educator_verification_requests set status=requested_decision, reviewed_at=final_at, review_notes=requested_notes, updated_at=now() where id=requested_id;
    when 'counselor' then
      ...
      if old_status not in ('pending','under_review') then raise exception 'request is not reviewable'; end if;
      update public.counselor_verification_requests set status=requested_decision, reviewed_at=final_at, review_notes=requested_notes, updated_at=now() where id=requested_id;
    when 'district' then
      ...
      if old_status not in ('pending','under_review') then raise exception 'request is not reviewable'; end if;
      update public.district_verification_requests set status=requested_decision, reviewed_at=final_at, review_notes=requested_notes, updated_at=now() where id=requested_id;
    when 'recruiting' then
      ...
      if old_status not in ('pending','under_review') then raise exception 'request is not reviewable'; end if;
      update public.recruiting_verification_requests set status=requested_decision, reviewed_at=final_at, review_notes=requested_notes, updated_at=now() where id=requested_id;
    when 'admissions' then
      ...
      if old_status not in ('pending','under_review') then raise exception 'request is not reviewable'; end if;
      update public.admissions_verification_requests set status=requested_decision, reviewed_at=final_at, review_notes=requested_notes, updated_at=now() where id=requested_id;
    when 'employer' then
      ...
      if old_status not in ('pending','under_review') then raise exception 'request is not reviewable'; end if;
      update public.employer_verification_requests set status=requested_decision, reviewed_at=final_at, review_notes=requested_notes, updated_at=now() where id=requested_id;
    when 'brand-partner' then
      ...
      update public.brand_partner_verification_requests
         set status=requested_decision,
             ...
             reviewed_at=final_at, review_notes=requested_notes, updated_at=now()
  ...
  insert into public.verification_review_events (
    ...
    old_status, requested_decision, requested_notes
  );
...
comment on function public.review_verification_request(text,uuid,text,text) is
  'Allowlisted Founder/Admin human-review transition for role verification/readiness evidence. Does not grant downstream Scholar-data authority.';

-- Phase 8 rich curriculum canonical authority.
-- Historical curriculum promotion is handled by a later conditional migration so
-- clean installs never depend on pre-canonical tables.
...
-- Completion remains the sole reward/credential authority, now with a server-side
-- checkpoint gate whenever the canonical module has an answer record.
...
begin
  ...
  if module_inserted then
    module_rewarded:=private.record_learning_reward(learner_id,'module.completed',requested_course_slug||':'||requested_module_key,course_row.coins_per_module,course_row.xp_per_module,'Completed '||module_row.title);
  end if;
  ...
  join public.learning_modules m on m.course_slug=p.course_slug and m.module_key=p.module_key and m.required=true

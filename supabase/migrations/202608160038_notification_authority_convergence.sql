...
begin
  if requested_owner_id is null
     or nullif(trim(requested_event_key),'') is null
     ...
     or nullif(trim(requested_title),'') is null
     or nullif(trim(requested_body),'') is null
     or requested_href !~ '^/'
     or requested_priority not in ('low','medium','high','urgent') then
  ...
  values (
    ...
    jsonb_build_object(
      'eventKey', requested_event_key,
      'type', requested_event_type,
      'title', left(trim(requested_title),160),
      'body', left(trim(requested_body),1000),
      'href', requested_href,
  ...
  on conflict (owner_id,event_key) do update
    ...
    where public.pbos_notification_outbox.state='PENDING'
  returning id into outbox_id;

  if outbox_id is null then
    select id into outbox_id from public.pbos_notification_outbox
      where owner_id=requested_owner_id and event_key=requested_event_key;
  end if;
  return outbox_id;
end;
$$;
...
begin
  ...
  if acknowledge_all then
    update public.pbos_notifications set read=true, acknowledged_at=coalesce(acknowledged_at,now())
      where user_id=auth.uid() and read=false;
  else
    if requested_notification_id is null then raise exception 'Notification ID required.' using errcode='22023'; end if;
    update public.pbos_notifications set read=true, acknowledged_at=coalesce(acknowledged_at,now())
      where id=requested_notification_id and user_id=auth.uid();
  end if;
...
begin
  if auth.uid() is null then raise exception 'Authentication required.' using errcode='42501'; end if;
  select state into current_state from public.pbos_notification_outbox
    where id=requested_outbox_id and owner_id=auth.uid() for update;
  ...
  update public.pbos_notification_outbox
     set state=requested_state,
         attempt_count=case when requested_state='FAILED' then attempt_count+1 else attempt_count end,
         last_error=case when requested_state='FAILED' then left(coalesce(requested_error,'Delivery failed'),500) else null end,
         next_attempt_at=requested_next_attempt_at,
         processed_at=case when requested_state='SUPPRESSED' then now() else null end
   where id=requested_outbox_id and owner_id=auth.uid();
end;
$$;
...
begin
  ...
  select * into outbox from public.pbos_notification_outbox
    where id=requested_outbox_id and owner_id=auth.uid() for update;
  ...
  if outbox.state='DELIVERED' then
    select * into saved from public.pbos_notifications where user_id=auth.uid() and source_event_key=outbox.event_key;
    return saved;
  end if;
  ...
  values(
    ...
    requested_priority,false,'in_app',outbox.event_key,coalesce(requested_provenance,'[]'::jsonb)
  )
  on conflict(user_id,source_event_key) do update
    set type=excluded.type,title=excluded.title,body=excluded.body,href=excluded.href,
        priority=excluded.priority,delivery_status='in_app',provenance=excluded.provenance
  returning * into saved;

  update public.pbos_notification_outbox
     set state='DELIVERED',processed_at=now(),last_error=null,next_attempt_at=null,attempt_count=attempt_count+1
   where id=outbox.id;
  return saved;
end;
$$;
...
create or replace function private.verification_destination(request_type text)
returns text language sql immutable set search_path='' as $$
  select case request_type
    when 'coach' then '/coach-os'
    when 'educator' then '/educator-os'
    when 'counselor' then '/counselor-os'
    when 'district' then '/district-os'
    when 'recruiting' then '/recruiting-os'
    when 'admissions' then '/admissions-os'
    when 'employer' then '/employer-os'
    when 'brand-partner' then '/brand-partner-os'
    when 'community-partner' then '/community-partner-os'
    when 'athlete-abroad' then '/athlete-abroad-os'
    else '/notifications'
  end;
$$;
...
begin
  perform private.enqueue_notification_event(
    new.subject_user_id,
    'verification:'||new.request_type||':'||new.request_id::text||':'||new.new_status,
    'verification',
    case new.new_status when 'approved' then 'Verification approved' when 'rejected' then 'Verification needs attention' else 'Verification review started' end,
    case new.new_status when 'approved' then 'Your '||replace(new.request_type,'-',' ')||' verification was approved.' when 'rejected' then 'Your '||replace(new.request_type,'-',' ')||' verification was not approved. Review the decision and next steps.' else 'A reviewer has started reviewing your verification evidence.' end,
    private.verification_destination(new.request_type),
    case when new.new_status='rejected' then 'high' else 'medium' end
  );
  return new;
end;
$$;
...
returns trigger language plpgsql security definer set search_path='' as $$
begin
  perform private.enqueue_notification_event(new.user_id,'credential:'||new.id::text,'milestone','New credential earned',new.credential_name||' is now part of your Playbook record.','/certificates','medium');
  return new;
end;
$$;

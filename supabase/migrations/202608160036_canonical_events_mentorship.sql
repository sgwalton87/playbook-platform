...
-- Reconciles the historical community_events/community_event_rsvps tables
-- forward instead of assuming they are new. Durable community operations replace
-- the old React-only event and mentorship arrays.
...
alter table public.community_events add column if not exists pillar text;
alter table public.community_events add column if not exists timezone text;
alter table public.community_events add column if not exists virtual_url text;
alter table public.community_events add column if not exists capacity integer;
alter table public.community_events add column if not exists xp_reward integer;
alter table public.community_events add column if not exists coin_reward integer;
alter table public.community_events add column if not exists status text;
alter table public.community_events add column if not exists updated_at timestamptz;

update public.community_events
set pillar = coalesce(nullif(trim(pillar),''), 'Community'),
    timezone = coalesce(nullif(trim(timezone),''), 'America/Los_Angeles'),
    xp_reward = coalesce(xp_reward,0),
    coin_reward = coalesce(coin_reward,0),
    status = coalesce(status, case when starts_at is not null and ends_at is not null and coalesce(visibility,'public')='public' then 'published' else 'draft' end),
    updated_at = coalesce(updated_at,created_at,now());

alter table public.community_events alter column pillar set default 'Community';
alter table public.community_events alter column timezone set default 'America/Los_Angeles';
alter table public.community_events alter column xp_reward set default 0;
alter table public.community_events alter column coin_reward set default 0;
alter table public.community_events alter column status set default 'draft';
alter table public.community_events alter column updated_at set default now();

alter table public.community_events drop constraint if exists community_events_capacity_check;
alter table public.community_events add constraint community_events_capacity_check check (capacity is null or capacity > 0) not valid;
alter table public.community_events validate constraint community_events_capacity_check;
alter table public.community_events drop constraint if exists community_events_xp_reward_check;
alter table public.community_events add constraint community_events_xp_reward_check check (coalesce(xp_reward,0) >= 0) not valid;
alter table public.community_events validate constraint community_events_xp_reward_check;
alter table public.community_events drop constraint if exists community_events_coin_reward_check;
alter table public.community_events add constraint community_events_coin_reward_check check (coalesce(coin_reward,0) >= 0) not valid;
alter table public.community_events validate constraint community_events_coin_reward_check;
alter table public.community_events drop constraint if exists community_events_status_check;
alter table public.community_events add constraint community_events_status_check check (status in ('draft','published','cancelled','completed')) not valid;
alter table public.community_events validate constraint community_events_status_check;
alter table public.community_events drop constraint if exists community_events_time_order_check;
alter table public.community_events add constraint community_events_time_order_check check (starts_at is null or ends_at is null or ends_at > starts_at) not valid;
alter table public.community_events validate constraint community_events_time_order_check;
...
alter table public.community_event_rsvps add column if not exists updated_at timestamptz;
update public.community_event_rsvps set updated_at=coalesce(updated_at,created_at,now());
alter table public.community_event_rsvps alter column updated_at set default now();
...
drop policy if exists "Read public events" on public.community_events;
drop policy if exists "Users manage own rsvps" on public.community_event_rsvps;
...
using (user_id = auth.uid() or exists (select 1 from public.mentor_circles c where c.id = circle_id and c.mentor_user_id = auth.uid()) or private.current_user_is_platform_operator());
...
create or replace function public.create_community_event(event_title text,event_description text,event_type_input text,pillar_input text,starts_at_input timestamptz,ends_at_input timestamptz,timezone_input text,location_input text,virtual_url_input text,capacity_input integer,xp_reward_input integer,coin_reward_input integer,publish_now boolean default true)
...
begin
  ...
  if event_type_input not in ('workshop','lab','civic','social','virtual','course','networking','community') then raise exception 'Unsupported event type.' using errcode='22023'; end if;
  if ends_at_input <= starts_at_input then raise exception 'Event end must be after start.' using errcode='22023'; end if;
  insert into public.community_events(title,description,event_type,pillar,starts_at,ends_at,timezone,location,virtual_url,capacity,xp_reward,coin_reward,status,created_by,updated_at)
  values(trim(event_title),trim(event_description),event_type_input,coalesce(nullif(trim(pillar_input),''),'Community'),starts_at_input,ends_at_input,coalesce(nullif(trim(timezone_input),''),'America/Los_Angeles'),nullif(trim(coalesce(location_input,'')),''),nullif(trim(coalesce(virtual_url_input,'')),''),capacity_input,greatest(coalesce(xp_reward_input,0),0),greatest(coalesce(coin_reward_input,0),0),case when publish_now then 'published' else 'draft' end,actor_id,now()) returning id into saved_id;
  return saved_id;
end;
$$;
...
returns uuid language plpgsql security definer set search_path='' as $$
declare actor_id uuid := auth.uid(); event_row public.community_events%rowtype; saved_id uuid; reward_already_issued boolean := false;
begin
  if actor_id is null or not private.current_user_is_platform_operator() then raise exception 'Platform operator authority required.' using errcode='42501'; end if;
  ...
  if attended_input and not reward_already_issued and (coalesce(event_row.coin_reward,0) > 0 or coalesce(event_row.xp_reward,0) > 0) then
    if private.record_learning_reward(requested_user_id,'event.attended',requested_event_id::text,coalesce(event_row.coin_reward,0),coalesce(event_row.xp_reward,0),'Verified attendance: ' || event_row.title) then
...
create or replace function public.create_mentor_circle(circle_name text,circle_description text,pillar_input text,mentor_user_id_input uuid,capacity_input integer default 12,next_session_at_input timestamptz default null,timezone_input text default 'America/Los_Angeles',location_input text default null)
returns uuid language plpgsql security definer set search_path='' as $$
declare actor_id uuid := auth.uid(); mentor_id uuid := coalesce(mentor_user_id_input,actor_id); saved_id uuid;
begin
  if actor_id is null then raise exception 'Authentication required.' using errcode='42501'; end if;
  ...
  if not exists (select 1 from public.profiles p where p.id=mentor_id and private.normalize_playbook_profile_role(coalesce(p.profile_mode,p.role,p.requested_role))='mentor' and p.onboarding_completed=true) then raise exception 'Mentor owner must be an onboarded Mentor.' using errcode='42501'; end if;
  if length(trim(coalesce(circle_name,''))) < 3 or length(trim(coalesce(circle_description,''))) < 10 then raise exception 'Circle name and description are required.' using errcode='22023'; end if;
  ...
  values(trim(circle_name),trim(circle_description),coalesce(nullif(trim(pillar_input),''),'Community'),mentor_id,greatest(coalesce(capacity_input,12),1),'active',next_session_at_input,coalesce(nullif(trim(timezone_input),''),'America/Los_Angeles'),nullif(trim(coalesce(location_input,'')),''),actor_id) returning id into saved_id;
  return saved_id;
end;
$$;
...
declare actor_id uuid := auth.uid(); circle_row public.mentor_circles%rowtype; count_active integer; next_status text; saved_id uuid;
begin
  if actor_id is null then raise exception 'Authentication required.' using errcode='42501'; end if;
  ...
  on conflict(circle_id,user_id) do update set status=excluded.status,joined_at=case when excluded.status in ('active','waitlisted') then now() else mentor_circle_memberships.joined_at end,ended_at=excluded.ended_at returning id into saved_id;
  select count(*) into count_active from public.mentor_circle_memberships m where m.circle_id=requested_circle_id and m.status='active';

  ...
  return (
    <PlaybookPage>
      ...
      <PlaybookCard eyebrow="Delivery" title="Choose how Playbook reaches you">
        <div style={preferencesGrid}>
          {preferenceTypes.map((type) => (
            <label key={type} style={preferenceLabel}>
        ...
        <PlaybookGrid min={340}>
          {visible.map((item) => (
            <PlaybookCard key={item.id} eyebrow={`${item.type.replaceAll("_", " ")} · ${item.priority}`} title={item.title}>
              ...
              <div style={actions}>
                <Link href={item.href} style={primaryLink}>Take action →</Link>
                {!item.read && <button type="button" onClick={() => void act({ action: "READ", notificationId: item.id })} style={secondaryButton}>Mark read</button>}
              </div>
            </PlaybookCard>
          ))}
        </PlaybookGrid>
      ...
      {failures.length > 0 && (
        <section style={recoverySection} aria-label="Delivery recovery">
          <h2 style={recoveryHeading}>Delivery recovery</h2>
          <PlaybookGrid min={320}>
            {failures.map((item) => <PlaybookCard key={item.id} eyebrow={`${item.attempt_count} attempts`} title={item.event_type}><p style={copy}>{item.last_error}</p><button type="button" onClick={() => void act({ action: "RETRY", outboxId: item.id })} style={secondaryButton}>Retry trusted delivery</button></PlaybookCard>)}
          </PlaybookGrid>
        </section>
      )}
    </PlaybookPage>

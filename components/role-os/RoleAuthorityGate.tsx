...
export default function RoleAuthorityGate({ roleOS, children }: { roleOS: PlaybookRoleOS; children: React.ReactNode }) {
  return <CanonicalRoleAuthorityGate role={OS_TO_ROLE[roleOS]}>{children}</CanonicalRoleAuthorityGate>;
}

export function CanonicalRoleAuthorityGate({ role, children }: { role: PlaybookRole; children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const relationships = Array.from(new Set((relationshipResult.data ?? []).map((relationship) => relationship.relationship)));
      setActiveRelationshipTypes(relationships);

      if (role === "family" && relationships.includes("parent_guardian")) {
        setState("active");
        return;
      }
  ...
  const familyDetail = role === "family"
    ? hasParentGuardianRelationship
      ? "Your governed Parent / Guardian relationship is active."
...
function RoleGateSurface({ role, title, body, message, children }: { role: PlaybookRole; title: string; body: string; message?: string | null; children?: React.ReactNode }) {
  return (
    <main data-visual-canon="PGDS-001" data-testid={`${role}-os-authority-gate`} style={page}>
      <section style={card}>
        ...
        {children}
        <div style={routeBadge}>Canonical destination: <strong>{PLAYBOOK_ROLES[role].osRoute}</strong></div>
      </section>
    </main>
  );
}

const page: React.CSSProperties = { minHeight: "100vh", display: "grid", placeItems: "center", padding: 24, background: "radial-gradient(circle at 80% 12%,rgba(249,115,22,.18),transparent 28%),linear-gradient(135deg,#06172D,#081D38 56%,#031023)", color: "#FFFFFF" };
const card: React.CSSProperties = { width: "100%", maxWidth: 860, padding: "clamp(28px,5vw,52px)", borderRadius: 28, border: "1px solid rgba(255,255,255,.16)", background: "rgba(4,18,39,.94)", boxShadow: "0 30px 90px rgba(0,0,0,.28)" };
const eyebrow: React.CSSProperties = { margin: 0, fontFamily: "'Space Mono', monospace", fontSize: 11, fontWeight: 900, letterSpacing: ".14em", textTransform: "uppercase", color: "#FF9A6C" };
const titleStyle: React.CSSProperties = { margin: "12px 0", fontSize: "clamp(34px,6vw,64px)", lineHeight: 1 };
const bodyStyle: React.CSSProperties = { margin: 0, color: "#C7D5E5", fontSize: 17, lineHeight: 1.65 };
const notice: React.CSSProperties = { marginTop: 20, padding: 16, borderRadius: 16, border: "1px solid rgba(255,154,108,.35)", background: "rgba(249,115,22,.12)", color: "#FFE7D6", lineHeight: 1.6 };
const routeBadge: React.CSSProperties = { marginTop: 18, display: "inline-block", padding: "8px 12px", borderRadius: 999, background: "rgba(255,255,255,.08)", color: "#C7D5E5", fontFamily: "'Space Mono', monospace", fontSize: 11 };

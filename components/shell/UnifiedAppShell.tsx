...
export default function UnifiedAppShell({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    async function loadIdentityAndContext() {
      ...
      if (contextResult.ok) {
        ...
        if (!active) return;
        setContext({
          evidenceCount: Number(body.evidenceCount ?? 0),
          pendingVerificationCount: Number(body.pendingVerificationCount ?? 0),
          unreadAttentionCount: Number(body.unreadAttentionCount ?? 0),
          activeSupportCount: Number(body.activeSupportCount ?? 0),
        });
      }
  ...
  return (
    <div className="playbook-app-shell" data-visual-canon="PGDS-001" data-playbook-surface="authenticated-product-shell"
      ...
      <aside className="playbook-sidebar" data-playbook-sidebar="true">
        ...
        <Link href={roleNav.home} className="playbook-brand">
          ...
          {open && <span><strong>Playbook OS</strong><small>{roleNav.label}</small></span>}
        </Link>

        <Link href="/profile" className="playbook-profile-card">
          <ProfileAvatar src={profile?.avatar_url} name={displayName} size={open ? 46 : 42} />
          {open && <span style={{ minWidth: 0 }}><strong className="playbook-profile-name">{displayName}</strong><small className="playbook-role-badge">{roleNav.label}</small></span>}
        </Link>

        {open && (
          <section aria-label="Your live Playbook context" style={contextPanel}>
        ...
        <button type="button" onClick={handleSignOut} className="playbook-signout" style={{ justifyContent: open ? "flex-start" : "center" }}
          disabled={signingOut} aria-describedby={signOutError ? "sign-out-error" : undefined}>
          <span aria-hidden="true">↪</span>{open && <span>{signingOut ? "Signing out…" : "Sign out"}</span>}
        </button>
      ...
      <section className="playbook-main">
        <header className="playbook-mobile-header">
          ...
          <button type="button" onClick={handleSignOut} className="playbook-signout" disabled={signingOut} aria-describedby={signOutError ? "sign-out-error" : undefined}>
        ...
        <header className="playbook-topbar">
          ...
          <div style={topbarContext}>
            <Link href="/notifications" style={attentionLink} aria-label={`${attentionTotal} items need attention`}>
              {attentionTotal > 0 ? `${attentionTotal} need attention` : "All caught up"}
            </Link>
            <span style={topbarFact}>{context.evidenceCount} evidence</span>
            <span style={topbarFact}>{context.activeSupportCount} support</span>
          </div>
...
function ShellNav({ items, pathname, compact = false }: { items: NavItem[]; pathname: string | null; compact?: boolean }) {
  return <nav>{items.map((item) => {
    const active = pathname === item.href || pathname?.startsWith(item.href + "/");
    return <Link key={item.href} href={item.href} className={`playbook-nav-item${active ? " playbook-nav-item--active" : ""}`}
      style={{ justifyContent: compact ? "center" : "flex-start" }} title={compact ? item.label : undefined}>
      <span aria-hidden="true">{item.icon}</span>{!compact && <span>{item.label}</span>}
    </Link>;
  })}</nav>;
}

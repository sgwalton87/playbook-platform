...
| `/onboarding` | `app/onboarding/page.tsx` | Root layout with shell unless app-shell excludes by policy (not in shell bypass list) | Legacy custom onboarding UI, `CollegeSearch`, `Confetti`, `next/image` | Supabase auth/profile/storage writes and profile updates in-page | Large local option arrays for sports, districts, cities, grades, income, etc. | Source is a complete client page, not a redirect-only page | Yes |
| `/tutorial` | `app/tutorial/page.tsx` | Root layout with shell | `OnboardingTour` | No Supabase/API writes; `OnboardingTour` calls pure `getFirstLoginTutorial()` and `getTutorialProgress(['home','record'])` | Hardcoded tutorial step list and hardcoded completed IDs | No auth guard in page; not linked from onboarding submit | Yes |
...
| `/scholar-athlete-os` | `app/scholar-athlete-os/page.tsx` | Root layout with shell | `ScholarAthleteDashboard` | No auth or profile read in wrapper or dashboard component | Hardcoded recruiting targets `Target University` and `Dream College`; static eligibility/NIL/action inputs | No auth guard | Yes |
...
- `/onboarding` remains a separate complete client page, so “legacy onboarding removed” is not true in the runtime source.
- `/tutorial` is reachable as an independent page but no traced onboarding submit path navigates there.
- Most role OS routes do not read the signed-in user profile at the route/component level; they render static or config-driven dashboards.

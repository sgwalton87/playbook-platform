...
export default function CertificatesPage() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (!auth.user) { router.replace("/login?next=/certificates"); return; }
  ...
  return <PlaybookPage>
    ...
    <PlaybookMetrics><PlaybookMetric label="Earned" value={loading ? "…" : String(credentials.length)} /><PlaybookMetric label="Remaining" value={loading ? "…" : String(locked.length)} /><PlaybookMetric label="Credential source" value="Learning" /></PlaybookMetrics>
    {error && <div role="alert" style={alert}>{error}</div>}
    {loading ? <div style={state}>Loading credential evidence…</div> : credentials.length === 0 ? <PlaybookCard eyebrow="Credential Vault" title="Your first credential is still ahead"><p style={copy}>Complete every required module in a published Playbook course. The credential is issued automatically by the governed learning transaction.</p><Link href="/courses" style={primaryLink}>Go to Courses →</Link></PlaybookCard> : <PlaybookGrid min={300}>{credentials.map((credential) => { const course = courses.find((item) => item.slug === credential.course_slug); return <PlaybookCard key={credential.id} eyebrow={course?.pillar || "Playbook Credential"} title={credential.credential_name}><div style={seal}>🎓</div><PlaybookPill>Verified completion</PlaybookPill><p style={copy}>Issued {new Date(credential.issued_at).toLocaleDateString()} · {Number(credential.evidence?.completed_modules || 0)} required modules recorded.</p><Link href={`/courses/${credential.course_slug}`} style={secondaryLink}>Review evidence</Link></PlaybookCard>; })}</PlaybookGrid>}
    {locked.length > 0 && <><section style={sectionHeader}><p style={eyebrow}>Still to earn</p><h2 style={heading}>Locked credentials</h2></section><PlaybookGrid min={280}>{locked.map((course) => <PlaybookCard key={course.slug} eyebrow={course.pillar} title={course.certificate_name}><PlaybookPill>{course.status === "coming_soon" ? "Coming soon" : "Locked"}</PlaybookPill><p style={copy}>{course.status === "coming_soon" ? "Curriculum is not published yet." : "Complete the required course modules to issue this credential."}</p>{course.status === "published" && <Link href={`/courses/${course.slug}`} style={secondaryLink}>Open course</Link>}</PlaybookCard>)}</PlaybookGrid></>}
  </PlaybookPage>;
}

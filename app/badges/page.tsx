...
export default function BadgesPage() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (!auth.user) { router.replace("/login?next=/badges"); return; }
  ...
  return <PlaybookPage>
    ...
    <PlaybookMetrics><PlaybookMetric label="Badges earned" value={loading ? "…" : String(badges.length)} /><PlaybookMetric label="Course badges" value={loading ? "…" : String(courseBadges.length)} /><PlaybookMetric label="Authority" value="Governed" /></PlaybookMetrics>
    {error && <div role="alert" style={alert}>{error}</div>}
    {loading ? <div style={state}>Loading achievement evidence…</div> : badges.length === 0 ? <PlaybookCard eyebrow="Badge Vault" title="No governed badges yet"><p style={copy}>Complete a published course to earn a course-completion badge. Additional milestone badges can be added only when their issuing lifecycle is governed and auditable.</p><Link href="/courses" style={primaryLink}>Go to Courses →</Link></PlaybookCard> : <PlaybookGrid min={290}>{badges.map((badge) => <PlaybookCard key={badge.id} eyebrow={badge.source_type} title={badge.badge_name}><div style={seal}>★</div><PlaybookPill>Evidence-backed</PlaybookPill><p style={copy}>{badge.description}</p><p style={meta}>Awarded {new Date(badge.awarded_at).toLocaleDateString()} · Source: {badge.source_id}</p>{badge.source_type === "course" && <Link href={`/courses/${badge.source_id}`} style={secondaryLink}>Open source course</Link>}</PlaybookCard>)}</PlaybookGrid>}
  </PlaybookPage>;
}

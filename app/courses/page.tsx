...
type Course = {
  slug:string; title:string; description:string; pillar:string; image_url:string|null;
  status:"published"|"coming_soon"; xp_per_module:number; coins_per_module:number;
  course_xp_bonus:number; course_coin_bonus:number; certificate_name:string;
  moduleCount:number; completedModules:number; completed:boolean;
...
export default function CoursesPage(){
  ...
  async function refreshCatalog(){
    ...
    try{setCourses(await loadCatalog());}
    catch(cause){setError(cause instanceof Error?cause.message:"Learning catalog could not be loaded.");}
  ...
  const pillars=Array.from(new Set(published.map(course=>course.pillar).filter(Boolean))).sort();
  const needle=query.trim().toLowerCase();
  const visiblePublished=published.filter(course=>(pillar==="all"||course.pillar===pillar)&&(!needle||`${course.title} ${course.description} ${course.pillar}`.toLowerCase().includes(needle)));
  ...
  return <PlaybookPage>
    ...
    {loading?<div style={state}>Loading canonical learning catalog…</div>:<>
      ...
      <section style={searchPanel} aria-label="Course search">
        <label style={searchLabel}>Search courses
          <input aria-label="Search courses" value={query} onChange={event=>setQuery(event.target.value)} placeholder="Search title, topic, or pillar" style={input}/>
        </label>
        <label style={searchLabel}>Pillar
          <select aria-label="Filter by pillar" value={pillar} onChange={event=>setPillar(event.target.value)} style={input}>
            <option value="all">All pillars</option>
            {pillars.map(value=><option key={value} value={value}>{value}</option>)}
          </select>
        </label>
        <span role="status" aria-live="polite" style={muted}>{visiblePublished.length} course{visiblePublished.length===1?"":"s"} found</span>
      </section>

      <section style={sectionHeader}><p style={eyebrow}>Published curriculum</p><h2 style={heading}>Your learning library</h2><p style={muted}>Every completion below is tied to durable module evidence and idempotent reward issuance.</p></section>

      {visiblePublished.length===0?<div style={state}>No published courses match your search.</div>:<PlaybookGrid min={310}>
        {visiblePublished.map(course=>{
          const percent=course.moduleCount?Math.round(course.completedModules/course.moduleCount*100):0;
          return <PlaybookCard key={course.slug} eyebrow={course.pillar} title={course.title}>
            ...
            <div style={rewardRow}><PlaybookPill>+{course.xp_per_module} XP/module</PlaybookPill><PlaybookPill>+{course.coins_per_module} coins/module</PlaybookPill>{course.credential&&<PlaybookPill>Credential earned</PlaybookPill>}</div>
      ...
      {comingSoon.length>0&&<><section style={sectionHeader}><p style={eyebrow}>Curriculum pipeline</p><h2 style={heading}>Coming next</h2></section><PlaybookGrid min={300}>{comingSoon.map(course=><PlaybookCard key={course.slug} eyebrow={course.pillar} title={course.title}><p style={copy}>{course.description}</p><PlaybookPill>Coming soon</PlaybookPill></PlaybookCard>)}</PlaybookGrid></>}
    </>}
  </PlaybookPage>;
}
...
const searchPanel:React.CSSProperties={maxWidth:1180,margin:"0 auto 24px",padding:16,borderRadius:18,background:"#FFFFFF",border:"1px solid #E2E8F0",display:"flex",gap:14,flexWrap:"wrap",alignItems:"end"};
const searchLabel:React.CSSProperties={display:"grid",gap:6,color:"#334155",fontWeight:800,flex:"1 1 240px"};
const input:React.CSSProperties={border:"1px solid #CBD5E1",borderRadius:12,padding:"11px 12px",background:"#FFFFFF",color:"#0F172A"};

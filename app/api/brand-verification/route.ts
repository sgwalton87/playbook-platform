...
export async function GET() {
  try {
    ...
    const request = await supabase.from("brand_partner_verification_requests").select("id,organization_name,partner_title,brand_category,partnership_goals,target_audience,monthly_budget_range,nil_acknowledgement,campaign_types,approval_contact,status,submitted_at,reviewed_at,review_notes").eq("brand_user_id", user.id).maybeSingle();
...
export async function POST() {
  try {
    ...
    const result = await supabase.from("brand_partner_verification_requests").upsert({
      ...
      campaign_types: evidence.campaignTypes, approval_contact: evidence.approvalContact, status: "pending",
      reviewed_at: null, review_notes: null, updated_at: new Date().toISOString(),
    }, { onConflict: "brand_user_id" }).select("id,status,submitted_at").single();
